...
def setup_cookbook_routes() -> APIRouter:
    ...
    def _auto_register_llm_endpoint(req: ServeRequest, remote: str | None, *, skip_pin: bool = False) -> str | None:
        ...
        # pinned makes the picker reflect the actual launch intent. Ollama is the
        # exception: `skip_pin` means verification was deferred (host daemon
        # unreachable at register time), so don't pin an unverified tag.
        if is_ollama_endpoint:
            pinned_models = [req.repo_id] if req.repo_id and not skip_pin else []
        else:
